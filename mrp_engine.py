"""
mrp_engine.py — Motor MRP con SQLite + sync GitHub
"""
from datetime import datetime, timedelta
from database import get_connection, sync_save
...
def agregar_receta(nombre, descripcion=""):
    with get_connection() as conn:
        cur = conn.execute("INSERT INTO recetas (nombre,descripcion) VALUES (?,?)", (nombre, descripcion))
        conn.commit(); sync_save()
        return cur.lastrowid

def agregar_ingrediente_receta(receta_id, ingrediente_id, cantidad):
    with get_connection() as conn:
        conn.execute("""INSERT OR REPLACE INTO receta_ingredientes (receta_id,ingrediente_id,cantidad)
                        VALUES (?,?,?)""", (receta_id, ingrediente_id, cantidad))
        conn.commit(); sync_save()

def agregar_caja(nombre, descripcion="", precio_venta=0):
    with get_connection() as conn:
        cur = conn.execute("INSERT INTO cajas (nombre,descripcion,precio_venta) VALUES (?,?,?)",
...
def agregar_menu(nombre, descripcion="", categoria="nivel_medio", racion=2):
    with get_connection() as conn:
        cur = conn.execute("INSERT INTO menus (nombre,descripcion,categoria,racion) VALUES (?,?,?,?)",
...
def calcular_costo_menu(menu_id, margen_pct=35, empaque=5.0):
    with get_connection() as conn:
        ...
        menu = dict(menu)
        recetas = conn.execute("""SELECT r.id,r.nombre FROM menu_recetas mr
                                  JOIN recetas r ON r.id=mr.receta_id WHERE mr.menu_id=?""",
                               (menu_id,)).fetchall()
    racion=menu["racion"]; costo_total=empaque; cal=0; prot=0; veg=0; detalle=[]
    for r in recetas:
        c=calcular_costo_receta(r["id"], porciones=racion)
        costo_total+=c["costo"]; cal+=c["calorias"]; prot+=c["proteinas"]; veg+=c["vegetales"]
        detalle.append({"receta":r["nombre"],"costo":c["costo"],"calorias":c["calorias"],
                        "proteinas":c["proteinas"],"vegetales":c["vegetales"]})
...
def comparar_menus(filtro_categoria=None, filtro_racion=None, margen_pct=35, empaque=5.0):
    with get_connection() as conn:
        sql="SELECT id FROM menus WHERE activo=1"; params=[]
        if filtro_categoria: sql+=" AND categoria=?"; params.append(filtro_categoria)
        if filtro_racion:    sql+=" AND racion=?";    params.append(filtro_racion)
        ids=[r["id"] for r in conn.execute(sql,params).fetchall()]
    resultados=[calcular_costo_menu(i,margen_pct,empaque) for i in ids]
...
def crear_pedido(cliente, fecha_entrega, items):
    ...
    with get_connection() as conn:
        cur=conn.execute("INSERT INTO pedidos (cliente,fecha_pedido,fecha_entrega) VALUES (?,?,?)",
                         (cliente,hoy,fecha_entrega))
        pedido_id=cur.lastrowid
        for it in items:
            conn.execute("INSERT INTO pedido_detalle (pedido_id,caja_id,cantidad,porciones) VALUES (?,?,?,?)",
...
def calcular_requerimientos():
    with get_connection() as conn:
        lineas=conn.execute("""SELECT pd.caja_id,pd.cantidad,pd.porciones
        ...
        for ln in lineas:
            ...
            recetas=conn.execute("SELECT receta_id FROM caja_recetas WHERE caja_id=?",
                                 (ln["caja_id"],)).fetchall()
            for rec in recetas:
                bom=conn.execute("""
                    SELECT ri.ingrediente_id,ri.cantidad,i.nombre,i.unidad,
                           i.stock_actual,i.stock_minimo,i.costo_unitario,i.proveedor_id,
                ...
                for row in bom:
                    ...
                    if iid not in reqs:
                        reqs[iid]={"nombre":row["nombre"],"unidad":row["unidad"],
...
def generar_ordenes_compra(reqs, buffer_pct=0.10):
    hoy=datetime.now(); por_proveedor={}
    ...
    with get_connection() as conn:
        for pv,items in por_proveedor.items():
            ...
            cur=conn.execute("""INSERT INTO ordenes_compra
                (proveedor_id,fecha_emision,fecha_entrega_esperada,total) VALUES (?,?,?,?)""",
                (pv,hoy.strftime("%Y-%m-%d"),fecha,total))
            oid=cur.lastrowid
            for i in items:
                conn.execute("""INSERT INTO orden_compra_detalle
                    (orden_id,ingrediente_id,cantidad,costo_unitario) VALUES (?,?,?,?)""",
